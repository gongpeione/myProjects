(function() {
	...
	function Aj(a, b) {
		for (var c = "https:" == window.location.protocol ? "https://" : "http://", e = 1; e < a.length; e++) {
			...
			case 5:
				...
				break;
			case 6:
				Y[0] && !Xc(Y[0]) && (a[e][0] = c + Y[0]), Y[1] && !Xc(Y[1]) && (a[e][1] = c + Y[1])
			}
		}
		Dh[0] = a;
		Eh[1] = b
	...
	function Bj() {
		for (var a = 0; a < fd.length; a++) if (fd[a] === this) {
			fd.splice(a, 1);
			break
		}
	}
	function Fh(a) {
		for (var b = Cj, c = 0; qb[c];) if (b -= qb[c][2], 0 <= b) c++;
		else break;
	...
	function Gj(a, b) {
		...
		} else Ij(b) || (b += ""), qb.push([a, b, a.length + b.length + 2])
	}
	function of(a) {
	...
	function rb(a, b) {
		...
		} else this.lat = new He(1, -1), this.lng = new Md(180, -180)
	}
	function ib(a, b) {
	...
	var Dl = function(a) {
		...
		yg = function(a) {
			for (var b in a) if (xg(a, b)) return !1;
			return !0
		},
		hi = function(a, b, c) {
			var e = [],
		...
		ga = function(a, b) {
			for (var c in a) if (xg(a, c) && !1 === b(a[c], c)) return !1
		},
		Tb = function(a, b) {
			var c = a.style;
	...
	var Pc = function(a, b, c) {
		...
		ud, qc, Ta, rc = 0,
		db, Hg;
	for (ud = Fg.length; ec < ud; ec++) if (qc = Fg[ec], -1 != tb.indexOf(qc) && (rc = ec + 1, Ta = RegExp(qc + "[ /]?([0-9]+(.[0-9]+)?)").exec(tb))) {
		db = parseFloat(Ta[1]);
		break
	}
	if (6 == rc) {
		if (Ta = /^mozilla\/.*gecko\/.*(minefield|shiretoko)[ /]?([0-9]+(.[0-9]+)?)/.exec(tb)) rc = 5, db = parseFloat(Ta[2]);
		if (Ta = /trident\/.*rv:([0-9]{1,}[\.0-9]{0,})/.exec(tb)) rc = 2, db = parseFloat(Ta[1])
	}
	1 == rc && (Ta = /^opera\/9.[89].*version\/?([0-9]+(.[0-9]+)?)/.exec(tb)) && (db = parseFloat(Ta[1]));
	ec = 0;
	for (ud = Gg.length; ec < ud; ec++) if (qc = Gg[ec], -1 != tb.indexOf(qc)) {
		Hg = ec + 1;
		break
	}
	var Bf = [rc, db, Hg],
	...
	var Mg = function(a) {
		...
		pe = function(a) {
			...
			var b = null,
				...
				d = a.offsetWidth,
				f = a.offsetHeight;
			if (a.getBoundingClientRect && !Wl) b = a.getBoundingClientRect(), a = Math.max(document.documentElement.scrollTop, document.body.scrollTop), c = Math.max(document.documentElement.scrollLeft, document.body.scrollLeft), c = b.left + c, e = b.top + a;
			else {
				if (document.getBoxObjectFor) b = document.getBoxObjectFor(a), c = a.style.borderLeftWidth ? parseInt(a.style.borderLeftWidth) : 0, e = a.style.borderTopWidth ? parseInt(a.style.borderTopWidth) : 0, c = b.x - c, e = b.y - e;
					...
					if (li || Xl && "absolute" == a.style.position) c -= document.body.offsetLeft, e -= document.body.offsetTop
				}
	...
	xa.getMouseContainerPixel = function(a) {
		var b = this._model;
		if (b) return b = b.get("map") || b, b = b.get("mapContainer") || b.getContainer(), b = pe(b), a = Mg(a), new Q(a[0] - b[0], a[1] - b[1])
	};
	...
	xa.keysUnReady = function(a, b, c) {
		pg(this, a, b, function(a, b) {
			...
			jb(a, function(a, e) {
				var f;
				if (c && ha(f = c(a, b[e])) ? f : null === a || Vb(a)) return d = !0, !1
			});
			return d
		})
	};
	...
	xi.draw = function() {
		var a = this.get("map"),
			...
			f = this.l;
		this.a = {};
		if (!a || !b || !1 === c) a = this.e, f && a && f.remove(a), qg(this.e);
		else {
	...
	var hp = function() {
		...
		If = {},
		kc;
	for (kc in Oc) if (Oc.hasOwnProperty(kc)) {
		var te = Oc[kc];
		te[0] && (Ug[te[0]] = !0);
		If[kc] = [];
		eb[kc] = eb[kc] || [];
			...
			if (!uc.hasOwnProperty(a)) {
				var c = Oc[a],
					...
					f = xl(c.length, function() {
						...
						for (c = d.length; c--;) if (f = d[c], Xg[f]) Xg[f]()
					});
				Xg[a] = f;
